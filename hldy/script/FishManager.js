  ...
  self.fishType = [
    {
      stype: 'l',
      images: [
        { name: 'l_1', width: 336, height: 84, frameNumber: 4 },
    ...
    },
    {
      stype: 'm',
      images: [
        { name: 'm_1', width: 280, height: 70, frameNumber: 4 },
    ...
    },
    {
      stype: 's',
      images: [
        { name: 's_1', width: 240, height: 30, frameNumber: 4 },
...
FishManager.prototype.setFishData = function () {
  ...
  for (var i in self.fishType) {
    for (var j in self.fishType[i].images) {
      ...
      self.fishData[image['name']] = new LBitmapData(
        dataList[image['name']],
        0,
        0,
        Math.floor(image.width / image.frameNumber),
        image.height,
      )
    }
  }
...
FishManager.prototype.setFish = function () {
  ...
  options.image =
    self.fishType[stypeIndex].images[Util.randomN(self.fishType[stypeIndex].images.length)]
  options.motion = {
    ...
    speed:
      (GAME_FPS *
        (LGlobal.width /
          Util.randomInRange(
            self.setFishRule[step].speedInSecond.min,
            self.setFishRule[step].speedInSecond.max,
          ))) /
      1000,
  }
