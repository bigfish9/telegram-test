...
Score.prototype.showScore = function (score) {
  var self = this
  var scoreTxt = window.GAME_LANG === 'en' ? 'Score: ' : '分数:'
  var s = scoreTxt + score.toString()
...
Score.prototype.showHooks = function (number) {
  ...
  if (number > originalHooksNumber) {
    for (var i = 0; i < number - originalHooksNumber; i++) {
      ...
      scoreHookBitmap.x =
        LGlobal.width - (i + originalHooksNumber + 1) * (self.scoreHookBitmapData.width * 1.2)
      scoreHookBitmap.y = 0
