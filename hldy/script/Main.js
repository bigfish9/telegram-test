...
var GAME_FPS = 30
init(1000 / GAME_FPS, 'gamediv', window.innerWidth * 0.99, window.innerHeight * 0.99, main)
var dataList
var stageLayer
var mySoundPlayer
var gScore = 0
...
var loadData = [
  ...
  { name: 'start_btn_zh', path: 'style/img_d/start_btn_zh.png' },
  { name: 'start_btn_en', path: 'style/img_d/start_btn_en.png' },
  { name: 'continue_btn_zh', path: 'style/img_d/continue_btn_zh.png' },
  { name: 'continue_btn_en', path: 'style/img_d/continue_btn_en.png' },
  { name: 'gohome_en', path: 'style/img_d/gohome_en.png' },
  { name: 'gohome_zh', path: 'style/img_d/gohome_zh.png' },
  ...
  { name: 'start_bg_zh', path: 'style/img_d/start_bg_zh.jpg' },
  { name: 'start_bg_en', path: 'style/img_d/start_bg_en.jpg' },
...
function gameStart() {
  ...
  window.parent.postMessage({ type: 'gameStart', from: 'game' }, '*')
  // return
  //debug
...
/**
 * 游戏结束处理函数
 * 负责清理游戏场景、显示游戏结束界面并上传分数
 */
function gameOver() {
  // 让当前游戏场景执行"死亡"操作，可能是停止动画、事件监听等
  stageLayer.die()
  // 清理当前游戏场景中的所有子对象，释放内存
  stageLayer.removeAllChild()
  // 清理所有LTweenLite动画实例，防止内存泄漏
  LTweenLite.removeAll()
  // 创建游戏结束界面
  var gameOvered = new GameOver()
  // 设置游戏结束界面的名称，方便后续查找和管理
  gameOvered.name = 'gameOver'
  // 将游戏结束界面添加到舞台层显示
  stageLayer.addChild(gameOvered)
  // 向父窗口发送游戏结束消息，包含游戏类型和分数信息
  window.parent.postMessage({ type: 'gameOver', from: 'game', score: gScore }, '*')
  // 结束函数执行
  return
}
...
Util.sFloat = function (object, duration) {
  originY = object.y
  LTweenLite.to(object, duration, { y: originY - 10, loop: true, ease: LEasing.None.easeIn }).to(
    object,
    duration,
    { y: originY + 10, loop: true, ease: LEasing.None.easeIn },
  )
}
